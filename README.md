...
### Example: Plot Trajectory in Python
...
df = pd.read_csv('config/trajectory_execution.csv')
...
plt.plot(df['time_seconds'], df['linear_velocity'], label='Linear Velocity')
plt.xlabel('Time (s)')
...
# Analyze curvature vs velocity
plt.figure(figsize=(10, 6))
plt.scatter(df['curvature'], df['linear_velocity'], alpha=0.5)
plt.xlabel('Curvature (1/m)')
plt.ylabel('Linear Velocity (m/s)')
plt.title('Velocity Adaptation Based on Curvature')
plt.grid(True)
plt.show()
```
...
Edit `scripts/path_executor.py` to tune performance:

```python
# Velocity limits
self.declare_parameter('max_linear_velocity', 0.7)      # m/s
self.declare_parameter('min_linear_velocity', 0.08)     # m/s
self.declare_parameter('max_angular_velocity', 2.84)    # rad/s

# Acceleration limits
self.declare_parameter('max_acceleration', 1.0)         # m/s²
self.declare_parameter('max_deceleration', 1.0)         # m/s²

# Control parameters
self.declare_parameter('look_ahead_distance', 0.3)      # m
self.declare_parameter('turn_look_ahead_distance', 1.0) # m
self.declare_parameter('velocity_smoothing_factor', 0.5) # 0-1
```

### Path Smoothing Parameters

Edit `scripts/path.py`:

```python
self.declare_parameter('num_samples', 500)              # Path resolution
self.declare_parameter('include_robot_position', True)  # Start from robot
self.declare_parameter('calculate_orientations', True)  # Tangent-based θ
```
